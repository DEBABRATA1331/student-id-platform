...
def search():
    ...
    if request.method == "POST":
        ...
        c = conn.cursor()
        if ieee_id:
            c.execute("SELECT id, name, ieee_id, Domain, [Joining_Date], Category, QR FROM students WHERE ieee_id = ?", (ieee_id,))
        else:
            c.execute("SELECT id, name, ieee_id, Domain, [Joining_Date], Category, QR FROM students WHERE name LIKE ?", (f"%{name}%",))
        row = c.fetchone()
        conn.close()
